...
def get_encoder(opt):
        if opt.backbone == "resnet":
            ...
            model = resnet_encoder_dlf.ResNet_SE(
                opt.num_layers, opt.num_layers, se_layers=opt.se_layers)
        return model
    ...
    if opt.ext_disp_to_eval is None:
        ...
        with torch.no_grad():
            for data in dataloader:
                ...
                if opt.using_inputs:
                    if opt.using_seg:
                        output = encoder["encoder"](input_color, input_seg)
                    else:
                        output = encoder["encoder"](input_color, input_color)
                else:
                    output = encoder["encoder"](input_color)
